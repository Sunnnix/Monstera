...
public class EditMonster extends JDialog {
	...
	private Field[] typeFields = de.snx.monstera.data.battle.Type.class.getDeclaredFields();
	...
	public EditMonster(Window win) {
		super(win, "Edit Monsters");
		this.win = win;
		for (Field field : typeFields)
			if (field.getType().equals(de.snx.monstera.data.battle.Type.class))
				try {
					typesS.add(((de.snx.monstera.data.battle.Type) field.get(null)).name);
	...
	private void a() {
		...
		main.add(monsterTypes = new JComboBox<String>(ProjectHandler.getMonsters().getAll()), BorderLayout.NORTH);
	...
	private void onMonsterSelected() {
		...
		MonsterType type = ProjectHandler.getMonsters().getValue(currentSelected);
	...
	private class AbilityHolder {
		...
		public String toString() {
			return "At Lv. " + level + "(" + ability.getName() + ")";
		}
	}
	...
	private void addMonster() {
		...
		if (name == null || name.isEmpty())
			return;
		int id = ProjectHandler.getMonsters().getNextID();
		currentSelected = id;
		MonsterType type = new MonsterType.Builder(id, name).build();
		ProjectHandler.getMonsters().putValue(type);
		monsterTypes.removeAllItems();
		String[] types = ProjectHandler.getMonsters().getAll();
	...
	private void save() {
		MonsterType.Builder builder = new MonsterType.Builder(Integer.parseInt(id.getText()), name.getText())
				.setType(de.snx.monstera.data.battle.Type.getTypeFromString((String) types1.getSelectedItem()),
						de.snx.monstera.data.battle.Type.getTypeFromString((String) types2.getSelectedItem()))
