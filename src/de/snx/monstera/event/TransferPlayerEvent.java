...
import static de.snx.monstera.event.TeleportEvent.*;
...
public class TransferPlayerEvent extends Event {
	...
	public void update(Map map, WorldState world, GameStateManager gsm) {
		if (phase == 1 && timer == 0) {
			ProjectHandler.getMaps().transferPlayer(mapID, x, y);
			if (direction != -1)
				ProjectHandler.getMaps().getPlayer().setDirection(direction);
		} else {
			if (timer == maxTime)
				if (phase == 0) {
					phase = 1;
	...
	public Pair<JPanel, Runnable> getEditorDialog(Window win) {
		...
		panel.add(maps = new JComboBox<>(ProjectHandler.getMaps().getMapsAsArray()), BorderLayout.CENTER);
		...
		Runnable apply = () -> {
			try {
				this.mapID = ((Map) maps.getSelectedItem()).getID();
