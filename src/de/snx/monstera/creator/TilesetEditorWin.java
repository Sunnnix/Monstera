...
public class TilesetEditorWin extends JDialog {
	...
	public TilesetEditorWin(CreatorWindow win) {
		super(win, "Tileset Editor", ModalityType.APPLICATION_MODAL);
		this.win = win;
	...
	private void init() {
		...
		addDelete.addActionListener(a -> {
			if (selected[0] == -1)
				return;
			if (selectedProp == null) {
				TilesetProperties.setPropertie(selected, new Propertie());
				loadPropertie();
			} else {
				TilesetProperties.removePropertie(selected);
				loadPropertie();
			}
		});
	...
	private JPanel createTilesetView() {
		JPanel panel = new JPanel() {
			...
			public void paint(Graphics g) {
				...
				BufferedImage tmp = win.tileset.getTileset(index);
				if (!tmp.equals(img)) {
					img = tmp;
	...
	private void loadPropertie() {
		...
		tf_src.setText("");
		selectedProp = TilesetProperties.getPropertie(selected);
