...
import de.snx.monstera.battle.Ability.Category;
...
public class EditAbilities extends JDialog {
	...
	public EditAbilities(CreatorWindow win) {
	...
	private void a() {
		abilities = new JComboBox<>(Ability.getAbilities());
		...
		panel.add(type = new JComboBox<>(de.snx.monstera.battle.Type.getTypes()));
	...
	private void onLoadAbility() {
		ability = Ability.getAbility((String) abilities.getSelectedItem());
		if (ability == null)
			return;
		name.setText(ability.name);
		desc.setText(ability.desc);
		type.setSelectedItem(ability.type.name);
		cat.setSelectedItem(ability.cat.name());
		power.setText(String.valueOf(ability.power));
		acc.setText(String.valueOf(ability.acc));
		ap.setText(String.valueOf(ability.AP));
		prio.setText(String.valueOf(ability.priority));
		critM.setText(String.valueOf(ability.critMultiplyer));
	}
	...
	private void addNew() {
		...
		if (name == null)
			return;
		Ability.addAbility(new Ability.Builder(name, de.snx.monstera.battle.Type.NORMAL).build(), null);
	...
	private void save() {
		...
		if (!checkValid(power, "Power") || !checkValid(acc, "Accuracy") || !checkValid(ap, "Uses")
			...
			return;
		Ability.addAbility(new Ability.Builder(name.getText(),
				de.snx.monstera.battle.Type.getTypeFromString((String) type.getSelectedItem()),
